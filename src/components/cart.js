...
import { removeCart, confirmOrder } from "../store";
import "./cart.css";
...
const Cart = () => {
  ...
  const { cartContain, cartConfirmed } = useSelector((state) => state.cart);

  const totalQuantity = cartContain.reduce(
    (acc, item) => acc + item.quantity,
    0
  );
  useEffect(() => {
    console.log(cartConfirmed);
  });

  const totalPrice = cartContain.reduce(
    (acc, item) => acc + item.quantity * item.price,
    0
  );

  const handleRemove = (item) => {
    dispatch(removeCart(item)); // Dispatch action to remove item
  };

  return (
    <div className="cart-container">
      <h2 className="totalQuantity">Your Cart ({totalQuantity})</h2>

      {cartContain.length > 0 ? (
        <>
          <div className="cart-items">
            {cartContain.map((item) => (
              <div key={item.id} className="cart-item">
                <div className="cart-item-info">
                  <p>{item.name}</p>
                  <div className="cart-amount">
                    <p style={{ color: "red" }}>{item.quantity}x</p>{" "}
