...
import "./confirmCart.css"; // Import the CSS file
import { useDispatch } from "react-redux";
import { useSelector } from "react-redux";
import { toggle } from "../store";
const OrderConfirmed = () => {
  let { cartConfirmed, isVisible } = useSelector((state) => state.cart);
  let dispatch = useDispatch();
  const orderTotal = cartConfirmed.reduce((acc, item) => acc + item.price, 0);
  ...
  return (
    <div className="order-container">
      ...
      <div className="order-summary">
        {cartConfirmed?.map((item) => (
          <div key={item.id} className="order-item">
            <img
              src={item.image.thumbnail} // Replace with actual item image URL
              alt={item.name}
      ...
      <button onClick={() => dispatch(toggle())} className="order-button">
        Start New Order
      </button>
    </div>
  );
